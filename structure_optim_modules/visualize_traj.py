"""
Script to load and visualize saved .traj files from structure optimization.
This script can visualize atomic structures, energy evolution, and animate trajectories.
"""
...
def main():
    ...
    if len(traj_files) > 1:
        ...
        fig_compare = compare_structures(traj_files)
        plt.show()
    ...
    if results:
        ...
        fig_best = visualize_single_structure(best_atoms, f"Best Structure ({best_file})")
        plt.show()
    ...
    for traj_file in traj_files[:3]:  # Limit to first 3 files
        fig_anim = animate_trajectory(traj_file)
        if fig_anim:
            plt.show()
